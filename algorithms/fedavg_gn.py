...
from tools import construct_lr_optimizer
...
class FedAvgGN():
    def __init__(
        ...
        if args.avg_n is False:
            self.init_bn = self.get_bn_state_dict(self.model)
            self.local_bns = {
                client: copy.deepcopy(self.init_bn) for client in self.clients
            }

        print("{}:{}".format("#" * 50, "FedAvgGN!"))

    def get_bn_state_dict(self, model):
        bn_state_dict = {}
        for name, param in model.state_dict().items():
            if "running" in name or "track" in name:
                bn_state_dict[name] = copy.deepcopy(param)
    ...
    def train(self):
        ...
        for r in range(1, self.args.max_round + 1):
            ...
            for client in sam_clients:
                ...
                if self.args.avg_n is False:
                    # do not use global bn stats
                    local_model.load_state_dict(
                        self.local_bns[client], strict=False
                    )
                ...
                # record local bn states
                """
                if self.args.avg_n is False:
                    self.local_bns[client] = copy.deepcopy(
                        self.get_bn_state_dict(local_model)
                    )
                """
            ...
            if self.args.avg_n is False:
                # remove bn stats
                self.model.load_state_dict(self.init_bn, strict=False)
    ...
    def update_local(self, r, model, train_loader, test_loader):
        ...
        optimizer = construct_lr_optimizer(
            model, lr, self.args
        )
