...
def construct_layer_lr_optimizer(model, lr, args):
    param_groups = []
    K = len(model.layers)

    for k, layer in enumerate(model.layers):
        coef = args.llr_a * (k + 1) / K + args.llr_b
        param_groups.append(
            {"params": layer.parameters(), "lr": coef * lr}
        )
...
def construct_lr_optimizer(model, lr, args):
    if args.optimizer == "SGD":
        optimizer = torch.optim.SGD(
            model.parameters(),
            lr=lr,
...
def construct_lr_scheduler(optimizer, args):
    ...
    elif args.scheduler == "WSConLR":
        # constant lr
        def lr_warm_start_con(t, T0=args.ws_step, T_max=args.epoches):
            # T0 = int(0.1 * T_max)
            if t <= T0:
